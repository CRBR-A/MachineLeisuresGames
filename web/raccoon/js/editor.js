    ...
    action: async () => rcn_download_file({
      ...
      content: await async function() {
        let scripts = await Promise.all([
          rcn_http_request('js/init.js'), // This needs to stay at the beginning
          rcn_http_request('js/audio.js'),
          rcn_http_request('js/bin.js'),
          rcn_http_request('js/canvas.js'),
          rcn_http_request('js/game.js'),
          rcn_http_request('js/gl.js'),
          rcn_http_request('js/utility.js'),
          rcn_http_request('js/vm.js'),
          rcn_http_request('js/vm_worker.js'),
        ]);
        let styles = await Promise.all([
          rcn_http_request('css/reset.css'),
          rcn_http_request('css/game.css'),
          rcn_http_request('css/export.css'),
        ]);

        let resources = await Promise.all([
          'img/control_axes.svg',
          'img/control_buttons.svg',
